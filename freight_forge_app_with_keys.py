...
if menu == "Register & Login":
    ...
    with tab1:
        ...
        with st.form("registration-form"):
            ...
            pan_gst = st.text_input("PAN/GST Number", key="pan_gst_number")
            business_type = st.selectbox("Business Type", ["Agriculture", "Logistics", "Other"], key="business_type")
            address = st.text_area("Business Address", key="business_address")
            username = st.text_input("Desired Username", key="desired_username")
            password = st.text_input("Password", type="password", key="password")
            doc = st.file_uploader("Upload Business Registration/ID Proof (PDF, JPG)", type=['pdf', 'jpg','jpeg','png'], key="upload_business_registration_id_proof_pdf_jpg_")

            if st.form_submit_button("Send OTP for Verification", key="send_otp_for_verification"):
...
if menu == "Freight Inquiry & Booking":
    ...
    if not user:
        ...
        with st.form("freight-inquiry"):
            ...
            goods_type = st.selectbox("Goods Type", ["Wheat","Corn","Soybean"], key="goods_type")
            qty = st.number_input("Quantity (metric tons)", min_value=1, max_value=1000, value=500, key="quantity_metric_tons_")
            origin = st.text_input("Origin", value="Quebec, QC", key="origin")
            destination = st.text_input("Destination", value="Windsor, ON", key="destination")
            dispatch_date = st.date_input("Preferred Dispatch Date", min_value=datetime.date.today(, key="preferred_dispatch_date"))
            submitted = st.form_submit_button("Check Rates & Wagon Options", key="check_rates_wagon_options")
        ...
        if submitted:
            ...
            option = st.radio("Select Option", [
                "Train A (Covered Hopper x25, departs 09:00, key="select_option")",
        ...
        if st.session_state.get('just_booked'):
            ...
            , key="download_pdf_waybill_demo_txt_")
...
if menu == "Track Shipment (Waybill)":
    ...
    ref = st.text_input("Enter Waybill Reference", key="enter_waybill_reference")
    if st.button("Track Now", key="track_now"):
        ...
        if waybill:
            ...
            if waybill['status'] != "Delivered" and st.button("Simulate Delivery", key="simulate_delivery"):
